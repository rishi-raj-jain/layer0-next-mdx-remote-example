...
export function SomePage({ mdxSource, componentNames }) {
  const components = {
    ...defaultComponents,
    SomeHeavyComponent: componentNames.includes('SomeHeavyComponent') ? SomeHeavyComponent : null,
  }
...
export async function getStaticProps() {
  ...
  const componentNames = [/<SomeHeavyComponent/.test(content) ? 'SomeHeavyComponent' : null].filter(Boolean)
